...
public class SUSEProductFactory extends HibernateFactory {
    ...
    public static List<SUSEProductSCCRepository> allProductRepos() {
        return  getSession().createNativeQuery("""
                                      SELECT * from suseProductSCCRepository
    ...
    public static List<SUSEProductSCCRepository> lookupPSRByChannelLabel(String channelLabel) {
        List<SUSEProductSCCRepository> products =  getSession().createNativeQuery("""
                                      SELECT * from suseProductSCCRepository
                                      WHERE channel_label = :channelLabel
                                      """, SUSEProductSCCRepository.class)
                .setParameter("channelLabel", channelLabel, StringType.INSTANCE)
                .getResultList();
    ...
    public static Optional<SUSEProductSCCRepository> lookupProductRepoByIds(long rootId, long productId, long repoId) {
        SUSEProductSCCRepository product;
        try {
            product =  getSession().createNativeQuery("""
                                      SELECT * from suseProductSCCRepository
                                      WHERE root_product_id = :rootid
                                      AND product_id = :productid
                                      AND repo_id = :repoid
                                      """, SUSEProductSCCRepository.class)
                    .setParameter("rootid", rootId, LongType.INSTANCE)
                    .setParameter("productid", productId, LongType.INSTANCE)
                    .setParameter("repoid", repoId, LongType.INSTANCE)
                    .getSingleResult();
        }
    ...
    public static List<SUSEProductSCCRepository> lookupByChannelName(String name) {
       return getSession().createNativeQuery("""
                                      ...
                                      """, SUSEProductSCCRepository.class)
                    .setParameter("channelname", name, StringType.INSTANCE)
               .getResultList();
    }
